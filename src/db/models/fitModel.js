...
import { fitSchema } from '../schemas/index.js';

const Fit = model('Fit', fitSchema);
...
class FitModel {
  ...
  // 어울림 document 객체 전체를 찾아오는 메소드
  findFits() {
    return Fit.find().lean(); // lean을 사용하여 POJO 객체로 바꿔준다.
  }

  // 특정 id를 _id로 갖는 어울림 document 객체를 찾아오는 메소드
  findById(id) {
    return Fit.findById(
  ...
  update(id, fit) {
    const updatedFit = Fit.findByIdAndUpdate(
      id, // document의 id, mongoDB에 저장된 _id에 저장된 값이다.
      fit,
      {
  ...
  // 특정 id를 _id로 갖고 있는 어울림 document를 삭제한다(hard delete).
  delete(id) {
    return Fit.findByIdAndDelete(
      id // document의 id, mongoDB에 저장된 _id에 저장된 값이다.
    ).lean(); // lean을 사용하여 POJO 객체로 바꿔준다.
  }
}
